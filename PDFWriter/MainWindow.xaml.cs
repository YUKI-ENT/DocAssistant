...
public partial class MainWindow : Window
{
    ...
    {
        FinishEditing();
        return !dirty || MessageBox.Show(this, "未保存の変更を破棄しますか？", "PDFWriter", MessageBoxButton.YesNo, MessageBoxImage.Question) == MessageBoxResult.Yes;
    }
    ...
    private async void SaveProject(object sender, RoutedEventArgs e)
    {
        ...
        FinishEditing();
        var dialog = new SaveFileDialog { Filter = "PDFWriter 編集データ|*.pdfwrite", DefaultExt = ".pdfwrite", InitialDirectory = settings.SaveFolder };
    ...
    {
        if (!CanReplace()) return;
        var dialog = new OpenFileDialog { Filter = "PDFWriter 編集データ|*.pdfwrite", InitialDirectory = settings.SaveFolder };
        if (dialog.ShowDialog(this) == true) await Guard(async () =>
        {
    ...
    private async void Print(object sender, RoutedEventArgs e)
    {
        ...
        await Guard(() =>
        {
            ...
            dialog.PrintDocument(BuildPrintDocument(dialog.PrintableAreaWidth, dialog.PrintableAreaHeight).DocumentPaginator, "PDFWriter");
