...
internal sealed partial class AccessSession : IDisposable
{
    ...
    public AccessSession()
    {
        var thread = new Thread(() =>
        {
            ...
            Dispatcher.Run();
        }) { IsBackground = true, Name = "PDFWriter Access COM" };
    ...
    public Task<AccessInspection> ConnectAsync(string path, string formName, bool openIfMissing = true) => RunAsync(() =>
    {
        ...
        try
        {
            ...
            if (candidate == null)
            {
                if (!openIfMissing) throw new InvalidOperationException("起動済みAccessに接続できません。先に32bit Accessでカルテを開き、AccessとPDFWriterを同じWindowsユーザー・同じ権限で実行してください。");
